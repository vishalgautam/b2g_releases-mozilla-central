...
MobileMessageDatabaseService.prototype = {
      ...
      while (currentVersion != event.newVersion) {
        switch (currentVersion) {
          ...
          case 6:
            ...
            break;
          case 7:
            if (DEBUG) debug("Upgrade to version 8. Including `messageClass` index.");
  ...
  upgradeSchema7: function upgradeSchema7(transaction) {
    ...
    objectStore.openCursor().onsuccess = function(event) {
      ...
      let record = cursor.value;
      let timestamp = record.timestamp;
      record.messageClassIndex = [record.messageClass, timestamp];
      cursor.update(record);
      cursor.continue();
    };
  },
  ...
  saveReceivedMessage: function saveReceivedMessage(
    ...
    let record = {
      ...
      messageClassIndex: [aMessageClass, aDate],
  ...
  saveSendingMessage: function saveSendingMessage(
    ...
    let record = {
      ...
      messageClassIndex: [MESSAGE_CLASS_NORMAL, aDate],
  ...
  createMessageList: function createMessageList(filter, reverse, aRequest) {
    ...
    this.newTxn(READ_ONLY, function (error, txn, store) {
      ...
      if (filter.delivery || filter.numbers || filter.messageClass ||
        ...
        let singleFilter;
        {
          ...
          if (filter.messageClass) numberOfContexts++;
        ...
        // Retrieve the keys from the 'messageClass' index that matches the
        // value of filter.messageClass.
        if (filter.messageClass) {
          if (DEBUG) debug("filter.messageClass " + filter.messageClass);
          createSimpleRangedRequest("messageClass", filter.messageClass);
        }
